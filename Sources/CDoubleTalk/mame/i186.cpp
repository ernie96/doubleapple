...
DEFINE_DEVICE_TYPE(I80C188EB, i80c188eb_cpu_device, "i80c188eb", "Intel 80C188EB")
...
i80c188eb_cpu_device::i80c188eb_cpu_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: i80186_cpu_device(mconfig, I80C188EB, tag, owner, clock, 8, true)
{
}
...
i80186_cpu_device::i80186_cpu_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock, int data_bus_size, bool eb_peripherals)
	: i8086_common_cpu_device(mconfig, type, tag, owner, clock)
	, m_eb_peripherals(eb_peripherals)
...
void i80186_cpu_device::device_start()
{
	...
	m_reloc = 0;
	memset(m_eb_registers, 0, sizeof(m_eb_registers));
	m_last_dma = 0;
	m_dma_latency = 0;
...
void i80186_cpu_device::device_reset()
{
	...
	m_reloc = m_eb_peripherals ? 0x00ff : 0x20ff;
	memset(m_eb_registers, 0, sizeof(m_eb_registers));
	m_mem.upper = 0xfffb;
...
void i80186_cpu_device::write_memory_byte_al(uint32_t addr)
{
	if ((m_reloc & 0x1000) && ((addr >> 8) & 0xfff) == (m_reloc & 0xfff))
		internal_port_w((addr >> 1) & 0x7f, (addr & 1) ? swapendian_int16(m_regs.w[AX]) : m_regs.w[AX]);
	else
		m_program->write_byte(addr, m_regs.b[AL]);
}
...
offs_t i80186_cpu_device::internal_port_offset(offs_t offset) const
{
	if (!m_eb_peripherals)
		return offset;

	// 80C186EB/80C188EB Peripheral Control Block byte offsets translated to
	// the functionally equivalent registers in the existing 80186 core.
	// Untranslated EB registers retain their own backing state below.
	switch (offset)
	{
	case 0x00: return 0x10; // interrupt vector
	case 0x01: return 0x11; // EOI
	case 0x02: return 0x12; // poll
	case 0x03: return 0x13; // poll status
	case 0x04: return 0x14; // interrupt mask
	case 0x05: return 0x15; // priority mask
	case 0x06: return 0x16; // in-service
	case 0x07: return 0x17; // request
	case 0x08: return 0x18; // interrupt status
	case 0x09: return 0x19; // timer interrupt control
	case 0x0c: return 0x1c; // INT0 control
	case 0x0d: return 0x1d; // INT1 control
	case 0x0e: return 0x1e; // INT2 control
	case 0x0f: return 0x1f; // INT3 control
	case 0x18: return 0x28; // timer 0 count
	case 0x19: return 0x29; // timer 0 compare A
	case 0x1a: return 0x2a; // timer 0 compare B
	case 0x1b: return 0x2b; // timer 0 control
	case 0x1c: return 0x2c; // timer 1 count
	case 0x1d: return 0x2d; // timer 1 compare A
	case 0x1e: return 0x2e; // timer 1 compare B
	case 0x1f: return 0x2f; // timer 1 control
	case 0x20: return 0x30; // timer 2 count
	case 0x21: return 0x31; // timer 2 compare A
	case 0x23: return 0x33; // timer 2 control
	case 0x54: return 0x7f; // relocation
	default:   return 0x80;
	}
}
...
{
	int temp, which;
	offs_t const original_offset = offset;
	offset = internal_port_offset(offset);
	if (offset == 0x80)
		return m_eb_registers[original_offset & 0x7f];
...
void i80186_cpu_device::internal_port_w(offs_t offset, uint16_t data)
{
	...
	if (offset == 0x80)
	{
		m_eb_registers[original_offset & 0x7f] = data;
		return;
	}
