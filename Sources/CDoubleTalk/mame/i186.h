...
DECLARE_DEVICE_TYPE(I80C188EB, i80c188eb_cpu_device)
...
protected:
	...
	i80186_cpu_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock, int data_bus_size, bool eb_peripherals = false);
	...
	virtual void write_memory_byte_al(uint32_t addr) override;
...
private:
	...
	offs_t internal_port_offset(offs_t offset) const;
	...
	int m_dma_latency;
	uint16_t m_reloc;
	bool const m_eb_peripherals;
	uint16_t m_eb_registers[0x80];
...
class i80c188eb_cpu_device : public i80186_cpu_device
{
public:
	// construction/destruction
	i80c188eb_cpu_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock);
};
